...
import { Upload, Target, Zap, Shield, CheckCircle, ArrowRight } from 'lucide-react';
import { useUploadResumeMutation } from '../features/api/resumeapi';
        ...
        <form onSubmit={handleSubmit} encType="multipart/form-data" className="grid lg:grid-cols-2 gap-8">
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-2xl shadow-sm border p-6">
              ...
              <button type="submit" disabled={(!resumeText && !resumeFile) || !jobRole || isLoading} className="w-full mt-6 bg-blue-600 text-white py-3 rounded-lg flex items-center justify-center space-x-2">
                {isLoading ? 'Uploading...' : 'Analyze Resume'} <Zap className="w-5 h-5"/>
              </button>
            </div>
